...
class NewSwinUnetv3RestorationNet(nn.Module):
    def __init__(self, ngf=64, n_blocks=16, groups=8, embed_dim=64, depths=(8,8), num_heads=(8,8), window_size=8, use_checkpoint=False, K=2):
        ...
        self.dyn_agg_restore = DynamicAggregationRestoration(ngf=ngf, n_blocks=n_blocks, groups=groups,
                                            ...
                                            window_size=window_size, use_checkpoint=use_checkpoint, K=K)
...
class DynamicAggregationRestoration(nn.Module):
    ...
    def forward(self, base, x, pre_offset_flow_sim, img_ref_feat):
        ...
        pre_offset = pre_offset_flow_sim[0]         #[9, K, 9, 40s, 40s, 2]
        pre_flow = pre_offset_flow_sim[1]           #[9, K, 40s, 40s, 2]
        pre_similarity = pre_offset_flow_sim[2]     #[9, K, 1, 40s, 40s]

        pre_relu1_swapped_feats = self.flow_warp(img_ref_feat['relu1_1'].unsqueeze(1).repeat(1,self.K,1,1,1).view(B*self.K, -1, H*4, W*4),
                                                pre_flow['relu1_1'].view(B*self.K, H*4, W*4, 2)).view(B, self.K, -1, H*4, W*4)      #[9*K, 64, 160, 160]
        pre_relu2_swapped_feats = self.flow_warp(img_ref_feat['relu2_1'].unsqueeze(1).repeat(1,self.K,1,1,1).view(B*self.K, -1, H*2, W*2),
                                                pre_flow['relu2_1'].view(B*self.K, H*2, W*2, 2)).view(B, self.K, -1, H*2, W*2)      #[9*K, 128, 80, 80]
        pre_relu3_swapped_feats = self.flow_warp(img_ref_feat['relu3_1'].unsqueeze(1).repeat(1,self.K,1,1,1).view(B*self.K, -1, H, W),
                                                pre_flow['relu3_1'].view(B*self.K, H, W, 2)).view(B, self.K, -1, H, W)              #[9*K, 256, 40, 40]

        # Unet
        x0 = self.unet_head(base)    # [9, 128, 160, 160]
        ...
        for k in range(self.K):
            ...
            down_relu1_offset = torch.cat([x0, pre_relu1_swapped_feat, img_ref_feat['relu1_1']], 1)     #[9, 256, 160, 160]
            down_relu1_offset = self.lrelu(self.down_large_offset_conv1(down_relu1_offset))             #[9, 64, 160, 160]
            down_relu1_offset = self.lrelu(self.down_large_offset_conv2(down_relu1_offset))             #[9, 64, 160, 160]

            down_relu1_swapped_feat.append(self.lrelu(self.down_large_dyn_agg([img_ref_feat['relu1_1'], down_relu1_offset],
                                pre_offset['relu1_1'][:,k,:,:,:], pre_similarity['relu1_1'][:,k,:,:,:])))       #[9, 64, 160, 160]

        down_relu1_weight = self.softmax(pre_similarity['relu1_1'])
        down_relu1_swapped_feat = (torch.stack(down_relu1_swapped_feat, dim=1) * down_relu1_weight).sum(dim=1)      #[9, 64, 160, 160]

        h = torch.cat([x0, down_relu1_swapped_feat], 1)     #[9, 192, 160, 160]
        h = self.down_head_large(h)                         #[9, 128, 160, 160]
        h = self.down_body_large(h) + x0                    #[9, 128, 160, 160]
        x1 = self.down_tail_large(h)                        #[B, 128, 80, 80]

        # medium scale
        down_relu2_swapped_feat = []
        for k in range(self.K):
            pre_relu2_swapped_feat = pre_relu2_swapped_feats[:, k, :, :, :]
            down_relu2_offset = torch.cat([x1, pre_relu2_swapped_feat, img_ref_feat['relu2_1']], 1) #[9, 384, 80, 80]
            down_relu2_offset = self.lrelu(self.down_medium_offset_conv1(down_relu2_offset))        #[9, 128, 80, 80]
            down_relu2_offset = self.lrelu(self.down_medium_offset_conv2(down_relu2_offset))        #[9, 128, 80, 80]
            down_relu2_swapped_feat.append(self.lrelu(
                    self.down_medium_dyn_agg([img_ref_feat['relu2_1'], down_relu2_offset],
                    pre_offset['relu2_1'][:,k,:,:,:], pre_similarity['relu2_1'][:,k,:,:,:])))       #[9, 128, 80, 80]

        down_relu2_weight = self.softmax(pre_similarity['relu2_1'])
        down_relu2_swapped_feat = (torch.stack(down_relu2_swapped_feat, dim=1) * down_relu2_weight).sum(dim=1)      #[9, 128, 80, 80]
        ...
        for k in range(self.K):
            ...
            relu3_offset = torch.cat([x2, pre_relu3_swapped_feat, img_ref_feat['relu3_1']], 1)      #[9, 640, 40, 40]
            relu3_offset = self.lrelu(self.up_small_offset_conv1(relu3_offset))                     #[9, 256, 40, 40]
            relu3_offset = self.lrelu(self.up_small_offset_conv2(relu3_offset))                     #[9, 256, 40, 40]
            relu3_swapped_feat.append(self.lrelu(
                    self.up_small_dyn_agg([img_ref_feat['relu3_1'], relu3_offset],
                    pre_offset['relu3_1'][:,k,:,:,:], pre_similarity['relu3_1'][:,k,:,:,:])))       #[9, 256, 40, 40]

        relu3_weight = self.softmax(pre_similarity['relu3_1'])
        relu3_swapped_feat = (torch.stack(relu3_swapped_feat, dim=1) * relu3_weight).sum(dim=1)     #[9, 256, 40, 40]

        # small scale
        h = torch.cat([x2, relu3_swapped_feat], 1)          #[9, 384, 40, 40]
        h = self.up_head_small(h)                           #[9, 128, 40, 40]
        h = self.up_body_small(h) + x2                      #[9, 128, 40, 40]
        x = self.up_tail_small(h)                           #[9, 128, 80, 80]

        # dynamic aggregation for relu2_1 reference feature
        relu2_swapped_feat = []
        for k in range(self.K):
            pre_relu2_swapped_feat = pre_relu2_swapped_feats[:, k, :, :, :]
            relu2_offset = torch.cat([x, pre_relu2_swapped_feat, img_ref_feat['relu2_1']], 1)       #[9, 384, 80, 80]
            relu2_offset = self.lrelu(self.up_medium_offset_conv1(relu2_offset))                    #[9, 128, 80, 80]
            relu2_offset = self.lrelu(self.up_medium_offset_conv2(relu2_offset))                    #[9, 128, 80, 80]
        ...
        relu2_swapped_feat = (torch.stack(relu2_swapped_feat, dim=1) * relu2_weight).sum(dim=1)     #[9, 256, 40, 40]
        ...
        for k in range(self.K):
            pre_relu1_swapped_feat = pre_relu1_swapped_feats[:, k, :, :, :]
            relu1_offset = torch.cat([x, pre_relu1_swapped_feat, img_ref_feat['relu1_1']], 1)       #[9, 256, 160, 160]
            relu1_offset = self.lrelu(self.up_large_offset_conv1(relu1_offset))                     #[9, 64, 160, 160]
            relu1_offset = self.lrelu(self.up_large_offset_conv2(relu1_offset))                     #[9, 64, 160, 160]
        ...
        relu1_swapped_feat = (torch.stack(relu1_swapped_feat, dim=1) * relu1_weight).sum(dim=1)     #[9, 256, 40, 40]

        # large scale
        h = torch.cat([x+x0, relu1_swapped_feat], 1)        #[9, 192, 160, 160]
        h = self.up_head_large(h)                           #[9, 128, 160, 160]
        h = self.up_body_large(h) + x                       #[9, 128, 160, 160]
        x = self.up_tail_large(h)                           #[9, 3, 160, 160]
