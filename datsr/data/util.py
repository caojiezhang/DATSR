...
from datsr.data.transforms import totensor
import pdb
...
def random_paths_from_folder(folders, keys, filename_tmpl):
    """Generate random paths from folders.
    ...
    assert len(input_paths) == len(ref_paths), (
        f'{input_key} and {ref_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(ref_paths)}.')
    for idx in range(len(ref_paths)):
        input_path = osp.join(input_folder, input_paths[idx])
        ref_path = osp.join(ref_folder, ref_paths[idx])
        paths.append(
            dict([(f'{input_key}_path', input_path),
                  (f'{ref_key}_path', ref_path)]))
    return paths
