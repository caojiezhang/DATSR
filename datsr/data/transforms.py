...
def mod_crop(img, scale):
    ...
    if img.ndim in [2, 3]:
        ...
        h_remainder, w_remainder = h % (scale*8), w % (scale*8)   # cjz
...
def paired_random_crop1(img_gts, img_lqs, gt_patch_size, scale, gt_path):
    """Paired random crop.
    ...
    h_lq, w_lq, _ = img_lqs[0].shape
    h_gt, w_gt, _ = img_gts[0].shape
    gt_patch_size_h, gt_patch_size_w = gt_patch_size[0], gt_patch_size[1]
    lq_patch_size_h, lq_patch_size_w = gt_patch_size_h // scale, gt_patch_size_w // scale
    ...
    # randomly choose top and left coordinates for lq patch
    top = random.randint(0, max(0, h_lq - lq_patch_size_h))
    left = random.randint(0, max(0, w_lq - lq_patch_size_w))
...
def random_crop(imgs, gt_patch_size):

    h_gt, w_gt, _ = imgs.shape

    # randomly choose top and left coordinates for patch
    top = random.randint(0, max(0, h_gt - gt_patch_size))
    left = random.randint(0, max(0, w_gt - gt_patch_size))

    # crop patch
    imgs = imgs[top: top+gt_patch_size, left:left+gt_patch_size, :]
